...
@app.route('/debug/restore-from-backup', methods=['POST'])
def restore_from_backup():
    """One-time migration: copy missing tickets, comments, assignees from restore DB."""
    from sqlalchemy import create_engine
    import json as _json

    RESTORE_URL = os.getenv('RESTORE_DATABASE_URL', '')
    if not RESTORE_URL:
        return {'error': 'RESTORE_DATABASE_URL env var not set'}, 400

    try:
        restore_engine = create_engine(RESTORE_URL)
        stats = {'users_added': 0, 'tickets_added': 0, 'comments_added': 0,
                 'assignees_added': 0, 'notifications_added': 0, 'errors': []}

        with restore_engine.connect() as src:
            # --- 1. Sync users: copy any users from restore DB missing in current DB ---
            current_user_ids = {r[0] for r in db.session.execute(text('SELECT id FROM users')).fetchall()}
            restore_users = src.execute(text('SELECT id, name, email, password_hash, role, department FROM users ORDER BY id')).fetchall()
            for u in restore_users:
                if u[0] not in current_user_ids:
                    try:
                        db.session.execute(text(
                            "INSERT INTO users (id, name, email, password_hash, role, department) "
                            "VALUES (:id, :name, :email, :ph, :role, :dept)"
                        ), {'id': u[0], 'name': u[1], 'email': u[2], 'ph': u[3], 'role': u[4], 'dept': u[5]})
                        stats['users_added'] += 1
                    except Exception as e:
                        stats['errors'].append(f"user {u[0]}: {e}")

            db.session.commit()

            # --- 2. Copy missing tickets ---
            current_ticket_ids = {r[0] for r in db.session.execute(text('SELECT id FROM tickets')).fetchall()}
            restore_tickets = src.execute(text(
                'SELECT id, subject, description, source, customer_name, contact, booking_id, '
                'team, priority, status, assigned_to, notify_emails, created_at, updated_at, '
                'resolved_at, resolved_by, lead_assignee_id FROM tickets ORDER BY id'
            )).fetchall()

            for t in restore_tickets:
                if t[0] not in current_ticket_ids:
                    # Verify lead_assignee_id exists
                    lead_id = t[16]
                    if lead_id:
                        exists = db.session.execute(text('SELECT 1 FROM users WHERE id = :uid'), {'uid': lead_id}).fetchone()
                        if not exists:
                            lead_id = None
                    try:
                        db.session.execute(text(
                            "INSERT INTO tickets (id, subject, description, source, customer_name, contact, "
                            "booking_id, team, priority, status, assigned_to, notify_emails, created_at, "
                            "updated_at, resolved_at, resolved_by, lead_assignee_id) "
                            "VALUES (:id, :subject, :desc, :source, :cname, :contact, :bid, :team, :pri, "
                            ":status, :assigned, :notify, :cat, :uat, :rat, :rby, :lead)"
                        ), {
                            'id': t[0], 'subject': t[1], 'desc': t[2], 'source': t[3],
                            'cname': t[4], 'contact': t[5], 'bid': t[6], 'team': t[7],
                            'pri': t[8], 'status': t[9], 'assigned': t[10], 'notify': t[11],
                            'cat': t[12], 'uat': t[13], 'rat': t[14], 'rby': t[15], 'lead': lead_id
                        })
                        stats['tickets_added'] += 1
                    except Exception as e:
                        stats['errors'].append(f"ticket {t[0]}: {e}")

            db.session.commit()

            # --- 3. Copy missing comments ---
            current_comment_ids = {r[0] for r in db.session.execute(text('SELECT id FROM comments')).fetchall()}
            restore_comments = src.execute(text(
                'SELECT id, ticket_id, author, body, created_at FROM comments ORDER BY id'
            )).fetchall()

            for c in restore_comments:
                if c[0] not in current_comment_ids:
                    # Only insert if the ticket exists in current DB
                    ticket_exists = db.session.execute(text('SELECT 1 FROM tickets WHERE id = :tid'), {'tid': c[1]}).fetchone()
                    if ticket_exists:
                        try:
                            db.session.execute(text(
                                "INSERT INTO comments (id, ticket_id, author, body, created_at) "
                                "VALUES (:id, :tid, :author, :body, :cat)"
                            ), {'id': c[0], 'tid': c[1], 'author': c[2], 'body': c[3], 'cat': c[4]})
                            stats['comments_added'] += 1
                        except Exception as e:
                            stats['errors'].append(f"comment {c[0]}: {e}")

            db.session.commit()

            # --- 4. Copy missing ticket_assignees ---
            current_assignees = {(r[0], r[1]) for r in db.session.execute(text('SELECT ticket_id, user_id FROM ticket_assignees')).fetchall()}
            restore_assignees = src.execute(text(
                'SELECT ticket_id, user_id FROM ticket_assignees'
            )).fetchall()

            for a in restore_assignees:
                if (a[0], a[1]) not in current_assignees:
                    # Only insert if both ticket and user exist
                    t_exists = db.session.execute(text('SELECT 1 FROM tickets WHERE id = :tid'), {'tid': a[0]}).fetchone()
                    u_exists = db.session.execute(text('SELECT 1 FROM users WHERE id = :uid'), {'uid': a[1]}).fetchone()
                    if t_exists and u_exists:
                        try:
                            db.session.execute(text(
                                "INSERT INTO ticket_assignees (ticket_id, user_id) VALUES (:tid, :uid)"
                            ), {'tid': a[0], 'uid': a[1]})
                            stats['assignees_added'] += 1
                        except Exception as e:
                            stats['errors'].append(f"assignee {a[0]}-{a[1]}: {e}")

            db.session.commit()

            # --- 5. Update the ticket ID sequence to the max ---
            max_tid = db.session.execute(text('SELECT MAX(id) FROM tickets')).scalar() or 1
            db.session.execute(text(f"SELECT setval('tickets_id_seq', {max_tid})"))
            max_cid = db.session.execute(text('SELECT MAX(id) FROM comments')).scalar() or 1
            db.session.execute(text(f"SELECT setval('comments_id_seq', {max_cid})"))
            db.session.commit()

        restore_engine.dispose()

        # Final counts
        final_tickets = db.session.execute(text('SELECT COUNT(*) FROM tickets')).scalar()
        final_max = db.session.execute(text('SELECT MAX(id) FROM tickets')).scalar()

        return {
            'status': 'success',
            'stats': stats,
            'final_ticket_count': final_tickets,
            'final_max_ticket_id': final_max,
        }
    except Exception as e:
        db.session.rollback()
        return {'error': str(e)}, 500
